    	...
    	<%@ include file="../public/header.jsp"%>
            <div class="middle">
                <div class="map">
                    ...
                    <span class="mapName">楼宇信息</span>
                </div>
                <!--除以下content内容，其余地方均为模板，不要改动-->
                <div class="content">
                    <div class="title">
                       	<div class="btn syncData" id="tongbu">同步数据</div>
                        <div class="dataDetails">同步61-MIS楼宇信息数据</div>
                    </div>
...
$("#tongbu").click(function(){
	if (confirm("确定更新楼宇信息吗?")) {
		...
		$(z).appendTo("body");
		$.get("web/building/refreshBuilding", function(data) {
			//alert(data.msg);
			$("#zccRES").remove();
			location.href="web/building/getBuilding";
		});
	}
});
...
function page(cuPage){
	...
	window.location.href = "web/building/getBuilding?pageIndex="+cuPage+"&pageSize=10";
	if(goPage != "" && goPage != null){
		window.location.href = "web/building/getBuilding?pageIndex="+goPage+"&pageSize=10";
	}
}
</script>
</body>
