    	...
    	<%@ include file="public/header.jsp"%>
            <div class="middle">
                <div class="map">
                    ...
                    <span class="mapName">支付方式</span>
                </div>
                <!--除以下content内容，其余地方均为模板，不要改动-->
                <div class="content">
                    <div class="title">
                       	<div class="btn syncData" id="tongbu">同步数据</div>
                        <div class="dataDetails">同步61-MIS支付方式数据</div>
                    </div>
                    <div class="tableContainer">
                        <table cellpadding="0" cellspacing="0">
                            ...
                            <tbody>
                            	<c:forEach var="xft" items="${paging.data}" varStatus="status">
									<tr>
										<td>${paging.startRowNum+(status.index)}</td>
										<td>${xft.xfTendercode}</td>
										<td>${xft.xfTenderdesc}</td>
										<td>${xft.rate}</td>
										<td>${xft.xfRefund==true?"是":"否"}</td>
										<td>${xft.isInvalid==1?"是":"否"}</td>
									</tr>
								</c:forEach>
                            </tbody>
                        </table>
...
$("#tongbu").click(function(){
	if (confirm("确定更新付款方式信息吗?")) {
		...
		$.ajax({
			...
			url : "web/xfTender/refreshTender",//(默认: 当前页地址) 发送请求的地址    
...
function page(cuPage){
	...
	window.location.href = "web/xfTender/getTender?pageIndex="+cuPage+"&pageSize=10";
	if(goPage != "" && goPage != null){
		window.location.href = "web/xfTender/getTender?pageIndex="+goPage+"&pageSize=10";
	}
}
</script>
</body>
